...
async def checkUserChannelPermissionDB(user_id, channel_id):
    with db_session:
        return select(count(ch) for ch in Channel if User[user_id] in ch.users and ch.id == channel_id)[:][0] == 1
...
async def addNewPostDB(user_id, channel_id, post: PostView):
    with db_session:
        if await checkUserChannelPermissionDB(user_id, channel_id):
            Post(text=post.text, send_time=post.send_time, channel=Channel[channel_id])
            commit()
            return True
        else:
            return False
...
async def getUserChannelsDB(user_id: int):
    with db_session:
        channels = []
        entities = select(ch for ch in Channel if User[user_id] in ch.users)[:]
...
async def decodeToken(token: str = Security(api_key_header)):
    ...
    if auth_hash == hmac.new(TELEGRAM_HASH, msg, sha256 ).hexdigest() and data['auth_date'] + AUTH_EXPIRES_IN * 4 >= unix_timestamp:
...
# 
@app.get('/')
async def hello():
    # await testDbFunc()
    return {"message": "Hello World!"}

# @app.get('/test_login', include_in_schema=False)
# async def testAuth(current_user: UserView = Security(decodeToken)):
#     return current_user
...
# channel section
@app.get('/channels')
async def getChannels(current_user: UserView = Security(decodeToken)):
...
async def getPosts(channel_id: int, current_user: UserView = Security(decodeToken)):
    posts = await getChannelPostsDB(current_user.id, channel_id)
    return posts

@app.post('/channels/{channel_id}')
async def addPosts(channel_id: int, post: PostView, current_user: UserView = Security(decodeToken)):
...
async def adminAuth(current_user: UserView = Security(decodeToken)):
    return current_user.id in ADMINS

@app.get('/admin/channels', include_in_schema=False)
async def getAllChannels(current_user: UserView = Security(decodeToken)):
    if current_user.id in ADMINS:
        return await getAllChannelsDB()

@app.get('/admin/posts', include_in_schema=False)
async def getAllPosts(current_user: UserView = Security(decodeToken)):
    if current_user.id in ADMINS:
        return await getAllPostsDB()

@app.get('/admin/users', include_in_schema=False)
async def getAllUsers(current_user: UserView = Security(decodeToken)):
    if current_user.id in ADMINS:
